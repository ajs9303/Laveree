...
export const initShop = (category) => {
  const grid = document.querySelector("#productsGrid");

  // 모바일 화면일 경우 topImage 변경
  ...
  if (mediaSize <= 768) {
    switch (category) {
      case "ALL":
        topImage.src = "/Laveree/image/shop/topImages/shop_all_mobile.jpg";
        break;
      case "REMOVER":
        topImage.src = "/Laveree/image/shop/topImages/shop_remover_mobile.jpg";
        break;
      case "GIFT":
        topImage.src = "/Laveree/image/shop/topImages/shop_gift_mobile.jpg";
        break;
    }
  }

  productLists
    .filter((v) => v.categoryType.includes(category))
    .forEach((v) => {
      ...
      v.stock == "sold-out"
        ? (prdLisStockSoldout.src =
            "/Laveree/image/shop/icons/productIcon_soldout.gif")
        : null;
